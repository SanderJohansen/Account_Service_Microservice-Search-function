...
public class CannabisDAO implements ICannabisDAO {
    ...
    public Cannabis getMultipleCannabis(String SearchTerm) throws DALException, SQLException {
        ...
        PreparedStatement getCannabis = connection.prepareStatement("SELECT * from Cannabis WHERE MATCH (CannabisName, idEffect, idSideEffect, idCatagory, Description) AGAINST (?)  ");
        getCannabis.setString(1, SearchTerm);
        ResultSet rs = getCannabis.executeQuery();
        if (rs.next()) {
            //dont know if proper way to do it
            //This has to be done with an array
            while(rs.next()) {
                int i = 1;
